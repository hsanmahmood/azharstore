...
import { LogOut, PanelLeft, Menu, Home, Package, Folder } from 'lucide-react';
...
const AdminLayout = () => {
  ...
  const navLinks = [
    ...
    { to: '/admin/categories', text: t('admin.nav.categories'), icon: Folder },
  ];
      ...
      <div className="flex flex-col h-full">
        <div className={`flex items-center justify-between p-4 mb-4 border-b border-brand-border/50`}>
          <div className={`flex items-center gap-3 transition-all duration-300 ${!isOpen ? 'opacity-0 w-0 h-0' : 'opacity-100'}`}>
            ...
            <span className="text-lg font-bold whitespace-nowrap">{user?.email}</span>
          </div>
        </div>
        ...
        <nav className="flex-grow px-2">
          <ul className="space-y-2">
            <li>
              <Link to="/admin" className={getNavLinkClasses(isOpen)('/admin/dashboard')} title={isOpen ? '' : t('admin.nav.home')}>
        ...
        <div className="px-2 py-4 mt-auto">
          <div className="border-t border-brand-border pt-4 space-y-2">
            <button onClick={() => setIsDesktopSidebarOpen(!isOpen)} className={`flex items-center w-full px-4 py-2.5 text-sm font-medium text-brand-secondary hover:bg-brand-primary/5 hover:text-brand-primary rounded-lg transition-colors duration-200 ${!isOpen ? 'justify-center' : ''}`}>
              ...
              <span className={`transition-opacity duration-200 whitespace-nowrap ${!isOpen ? 'hidden' : 'delay-200'}`}>{t('admin.nav.toggleSidebar')}</span>
            </button>
